...
static void print_setting(const char *setting_name, GVariant *setting)
{
    GVariantIter iter;
...
static void print_connection(const char *path)
{
    GDBusProxy *proxy;
    GError *error = NULL;
    ...
    {
        if (strcmp(setting_name, NM_SETTING_CONNECTION_SETTING_NAME) != 0 && strcmp(setting_name, type) != 0)
...
static void get_active_connection_details(const char *obj_path)
{
...
static void get_active_connections(GDBusProxy *proxy)
{
    GError *error = NULL;
