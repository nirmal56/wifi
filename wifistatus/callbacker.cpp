...
class networkStatusCheck
{
    ...
    void Init(callback ret_fun, GDBusProxy * proxy, gchar * sender, gchar * signal, GVariant * var, gpointer usrdata){
        ret_fun(proxy,sender,signal,var,usrdata);
    }
    char * nm_state_to_string(NMState state)
    {
        switch (state)
        {
                        ...
                        gpointer user_data)
    {
        ...
        {
            g_print("total childs in parameter is:%ld\n", g_variant_n_children(parameters));
...
int main(int argc, char *argv[])
{
    ...
    auto callback = std::bind(&networkStatusCheck::on_call,networkStatusCheck(),std::placeholders::_1,std::placeholders::_2,std::placeholders::_3,std::placeholders::_4,std::placeholders::_5);
